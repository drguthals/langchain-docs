...
### `MessagesState`
Since managing a list of chat messages is so common, LangGraph provides a pre-built state class. It uses the `add_messages` reducer, which handles appending new messages and updating existing ones by ID. Learn more about [advanced messaging below](#messages-and-messagestate).
...
- **Active Threads (Not Interrupted)**: You can change the entire topology (add/remove/rename nodes and edges) freely.
